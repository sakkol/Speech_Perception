function SP_POS_equalpeaks(Sbj_Metadata)
% This function streamlines calculation of phase opposition values by:
% loading peakRate and peakEnv locked epoched wavelet, gets equal number of
% events(trials) for peakRate and peakEnv events (this ensures same numbers
% are compared) calculates p-values from phase opposition sum from z-score,
% calculate ITPCz nd plots ITPCzs, p-values and ITPZ differences (with
% significant periods circled). 
...
save_folder = fullfile(Sbj_Metadata.results, strjoin(control_blocks,'_'), 'POS_equalpeaks');
...
for pp=1:2 % loop peakRate and peakEnv
    ...
    if pp==1 % peakRate events
        ...
        if size(fouri_of_words.corr_rspn_fouri_peakRate{1},1) > size(fouri_of_words.no_rspn_fouri_peakRate{1},1)
            nt = randperm(size(fouri_of_words.corr_rspn_fouri_peakRate{1},1),size(fouri_of_words.no_rspn_fouri_peakRate{1},1));
            fouri_of_words.corr_rspn_fouri_peakRate{1} = fouri_of_words.corr_rspn_fouri_peakRate{1}(nt,:,:,:);
        else
            nt = randperm(size(fouri_of_words.no_rspn_fouri_peakRate{1},1),size(fouri_of_words.corr_rspn_fouri_peakRate{1},1));
            fouri_of_words.no_rspn_fouri_peakRate{1} = fouri_of_words.no_rspn_fouri_peakRate{1}(nt,:,:,:);
        end
        ...
        % equalize number of events(trials)
        if size(fouri_of_words.corr_rspn_fouri_peakEnv{1},1) > size(fouri_of_words.no_rspn_fouri_peakEnv{1},1)
            nt = randperm(size(fouri_of_words.corr_rspn_fouri_peakEnv{1},1),size(fouri_of_words.no_rspn_fouri_peakEnv{1},1));
            fouri_of_words.corr_rspn_fouri_peakEnv{1} = fouri_of_words.corr_rspn_fouri_peakEnv{1}(nt,:,:,:);
        else
            nt = randperm(size(fouri_of_words.no_rspn_fouri_peakEnv{1},1),size(fouri_of_words.corr_rspn_fouri_peakEnv{1},1));
            fouri_of_words.no_rspn_fouri_peakEnv{1} = fouri_of_words.no_rspn_fouri_peakEnv{1}(nt,:,:,:);
        end
...
%             clim_v = get(gca,'clim');  %colorbar;
            ...
            if cond == 1 && pp ==1
                title(['Random ' num2str(size(fouri_of_words.corr_rspn_fouri_peakRate{1},1)) ' events during correct responses']);
            elseif cond == 2 && pp ==1
                title(['Random ' num2str(size(fouri_of_words.no_rspn_fouri_peakRate{1},1)) ' events during no-responses']);
            elseif cond == 1 && pp == 2
                title(['Random ' num2str(size(fouri_of_words.corr_rspn_fouri_peakEnv{1},1)) ' events during correct responses']);
            elseif cond == 2 && pp == 2
                title(['Random ' num2str(size(fouri_of_words.no_rspn_fouri_peakEnv{1},1)) ' events during no-responses']);
            end
