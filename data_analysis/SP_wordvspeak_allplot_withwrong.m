function SP_wordvspeak_allplot_withwrong(Sbj_Metadata)
...
% response)-POS).
% This one includes wrong responses with no responses too
...
for t = 1:size(control_events,1)
    for w = 1:5
        ...
        elseif ~strcmp(control_events.word_info{t}.response{w},'1')
...
save_folder = fullfile(Sbj_Metadata.results, strjoin(control_blocks,'_'), 'wordvspeak_allplot_wWrong');
if ~exist(save_folder,'dir'),mkdir(save_folder),end
bwr = load('bwr_cmap.mat');

plh = 3; % plot height: word onset locked, peakRate, peakEnv
plw = 5; % plot width: ERP, HFA, ITPC-correct, ITPC-no+wrongresp, POS
...
for el = 1:length(control_wlt.label)
    ...
    % plot average of corr&no+wrong - ERP - word
    ...
    title({'ERP (mean+stderr)';[num2str(size(accr_word_erp,1)) ' correct response - ' num2str(size(wrong_word_erp,1)) ' no+wrong response']})
    xlim([time_ERP(1) time_ERP(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylim,'k')
    plot(xlim,[0 0],'k')
    legend({'Correct','no+wrong resp'})
    
    % plot average of corr&no+wrong - ERP - pR
    ...
    shadedErrorBar(time_ERP,mean(for_avg,1),stderr(for_avg),'lineprops','r');
    title({'';[num2str(size(accr_pR_erp,1)) ' correct response - ' num2str(size(wrong_pR_erp,1)) ' no+wrong response']})
    xlim([time_ERP(1) time_ERP(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylim,'k')
    plot(xlim,[0 0],'k')
    ...
    % plot average of corr&no+wrong - ERP - pE
    ...
    shadedErrorBar(time_ERP,mean(for_avg,1),stderr(for_avg),'lineprops','r');
    title({'';[num2str(size(accr_pE_erp,1)) ' correct response - ' num2str(size(wrong_pE_erp,1)) ' no+wrong response']})
    xlim([time_ERP(1) time_ERP(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylim,'k')
    plot(xlim,[0 0],'k')
    ...
    % plot average of corr&no+wrong - HFA - word
    ...
    title({'HFA (mean+stderr)';[num2str(size(accr_word_hfa,1)) ' correct response - ' num2str(size(wrong_word_hfa,1)) ' no+wrong response']})
    ...
    % plot average of corr&no+wrong - HFA - pR
    ...
    shadedErrorBar(time_hfa,mean(for_avg,1),stderr(for_avg),'lineprops','r');
    title({'';[num2str(size(accr_pR_hfa,1)) ' correct response - ' num2str(size(wrong_pR_hfa,1)) ' no+wrong response']})
    xlim([time_hfa(1) time_hfa(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylims,'k')
    plot(xlim,[1 1],'k')
    ...
    % plot average of corr&no+wrong - HFA - pE
    ...
    shadedErrorBar(time_hfa,mean(for_avg,1),stderr(for_avg),'lineprops','r');
    title({'';[num2str(size(accr_pE_hfa,1)) ' correct response - ' num2str(size(wrong_pE_hfa,1)) ' no+wrong response']})
    xlim([time_hfa(1) time_hfa(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylims,'k')
    plot(xlim,[1 1],'k')
    ...
    for pp = 1:3 % first word onset, second peakRate, third peakEnv
        for cond = 1:2 % loop columns: first correct, second no+wrong response
            ...
            elseif cond == 2 && pp == 1
                title({'ITPCz - no+wrong response events';['Random ' num2str(size(wrong_word_tf,1)) ' events']});
            elseif cond == 1 && pp == 2
    ...
    print(fullfile(save_folder,[curr_label , '_greatPlot_wWord.jpg']),'-djpeg','-r300')
    close all
