function SP_BLP_comp(Sbj_Metadata,control_blocks,runagain,onlySuccess)
...
% baseline correct time-freq map, compute bandlimited power using
% my_BLP_butter.m from EPIPROC, then plots baseline corrected spectrogram,
% HFA from baseline corrected spectrogram and HFA from BLP function in a
% 1x3 figure.
...
if ~ismember(vars,'onlySuccess')
    onlySuccess = 0;
    save_folder = fullfile(Sbj_Metadata.results, strjoin(control_blocks,'_'),'BLP_HFA');
elseif onlySuccess == 1
    save_folder = fullfile(Sbj_Metadata.results, strjoin(control_blocks,'_'),'BLP_HFA_onlySuccess');
...
%% run BLP on ERP for HFA
% Band Limited Power frequencies
hgb_frequencies = [50 58; 62 70; 70 80; 80 90; 90 100; 100 110; 110 118; 122 130; 130 140; 140 150];

BLP_HFA = my_BLP_butter(control_ERP,hgb_frequencies,0);
...
for el = 1:size(control_ERP.trial,2) % loop electrodes
    ...
    % plot HFA from wavelet averaging 
    ...
    title('HFA from baseline corrected spectrogram')
    xlim([time_spec(1) time_spec(end)])
    ylim([-8 8])
    plot(xlim,[0 0],'k')
    set(gca, 'FontSize',13,'FontWeight','bold');
    ...
    sgtitle({['Elec: ' info.channelinfo.Label{el} ' - spectrograms; ' num2str(size(control_wlt.powspctrm,1)) ' trials'];...
        ['spectrogram baseline correction in ' num2str(baseline(1)) ' - ' num2str(baseline(2))]}, 'FontSize',15,'FontWeight','bold')
