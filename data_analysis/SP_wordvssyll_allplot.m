function SP_wordvssyll_allplot(Sbj_Metadata)
% Similar to SP_wordvspeak_allplot function, but only plots word and
% syllable onset locked events.
...
% locks) and 5 columns (ERP-HFA-ITPC(correct)-ITPC(no response)-POS).
...
save_folder = fullfile(Sbj_Metadata.results, strjoin(control_blocks,'_'), 'WrdSyll_allplot');
...
for el = 1:length(control_wlt.label)
    ...
    % plot average of corr&no - ERP - syll
    ...
    shadedErrorBar(time_ERP,mean(for_avg,1),stderr(for_avg),'lineprops','r');
    title({'';[num2str(size(accr_syll_erp,1)) ' correct response - ' num2str(size(wrong_syll_erp,1)) ' no response']})
    xlim([time_ERP(1) time_ERP(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylim,'k')
    plot(xlim,[0 0],'k')
    ...
    % plot average of corr&no - HFA - syll
    ...
    shadedErrorBar(time_hfa,mean(for_avg,1),stderr(for_avg),'lineprops','r');
    title({'';[num2str(size(accr_syll_hfa,1)) ' correct response - ' num2str(size(wrong_syll_hfa,1)) ' no response']})
    xlim([time_hfa(1) time_hfa(end)])
    set(gca, 'FontSize',12);
    plot([0 0], ylims,'k')
    plot(xlim,[1 1],'k')
    ...
    print(fullfile(save_folder,[curr_label , '_great_WrdSyll.jpg']),'-djpeg','-r300')
    close all
