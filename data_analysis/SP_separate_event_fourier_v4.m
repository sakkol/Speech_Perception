function SP_separate_event_fourier_v4(Sbj_Metadata,freq_bands,name)
...
if ~ismember(vars,'name')
    name = num2str(size(freq_bands));
end
clear vars
...
for f = 1:length(freq_bands)
    ...
    cl_freqs = nearest(control_wlt.freq, freq_bands{f}(1)):nearest(control_wlt.freq, freq_bands{f}(2));
...
%% Save v4
save_dir = fullfile(Sbj_Metadata.results, [strjoin(control_blocks,'_') '_v4']);
if ~exist(save_dir,'dir'),mkdir(save_dir),end
fprintf('Saving ''fouri_of_words'' to:\n-->%s\n',fullfile(save_dir, [strjoin(control_blocks,'_') '_ctrl_word_fouri' name '.mat']))
save(fullfile(save_dir, [strjoin(control_blocks,'_') '_ctrl_word_fouri' name '.mat']),'fouri_of_words','-v7.3');
